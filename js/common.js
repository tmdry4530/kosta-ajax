  ...
  app.ui = $.extend(app.ui || {}, {
    ...
    renderManualLocationForm: function($container, options) {
      ...
      var html = [
        ...
        '      <input id="manual-latitude" class="input-field" type="number" step="0.0001" inputmode="decimal" name="latitude" value="' + app.utils.escapeHtml(settings.lat) + '" aria-describedby="manual-latitude-hint" required>',
        '      <p id="manual-latitude-hint" class="field-hint">예: 37.3943 · 소수점 넷째 자리 정도까지 입력하면 충분해요.</p>',
        '    </div>',
        '    <div class="field-group">',
        '      <label class="field-label" for="manual-longitude">경도</label>',
        '      <input id="manual-longitude" class="input-field" type="number" step="0.0001" inputmode="decimal" name="longitude" value="' + app.utils.escapeHtml(settings.lon) + '" aria-describedby="manual-longitude-hint" required>',
        '      <p id="manual-longitude-hint" class="field-hint">예: 126.9568 · 주소 검색은 추후 지원 예정이며, 지금은 좌표 입력으로 위치를 보정할 수 있어요.</p>',
        '    </div>',
